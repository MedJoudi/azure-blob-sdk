 ...
 public class Program
 {
     ...
     private const string blobServiceEndpoint = "<blob-service-endpoint>";
    ...
    public static async Task Main(string[] args)
    {
        ...
        await Console.Out.WriteLineAsync($"Account HNE:\t{info?.IsHierarchicalNamespaceEnabled}");
        
        /* To invoke the EnumerateContainersAsync method, 
        passing in the serviceClient variable as a parameter */
        await EnumerateContainersAsync(serviceClient);

        //Searching Blobs in container "existingContainerName"
        ...
        //Searching Blobs in the new created container 'vector-graphics'
        await EnumerateBlobsAsync(serviceClient, newContainerName);


        //Search for a specific Blob in 'ContainerClient => vector-graphics'
    ...
    //List Blobs inside a specific container
    ...
    //Created a new container if it dosn't exist
    ...
    {      
        ...
        if (!exists)
        {
            await Console.Out.WriteLineAsync($"Blob {blob.Name} not found in \"{client.Name}\"!");
                
        }
        else
            await Console.Out.WriteLineAsync($"Blob Found in \"{client.Name}\", URI:\t{blob.Uri}");
        return blob;
    }
 }